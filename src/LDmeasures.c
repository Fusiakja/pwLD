...
double Dprime( double tab[3][3])
{
	...
	/* Dmax */
	if(D < 0) Dmax = fmin2( tab[0][2]*tab[2][0], tab[2][1]*tab[1][2]);
	else Dmax = fmin2( tab[0][2]*tab[2][1], tab[1][2]*tab[2][0]  );

	/*D'*/
	if(D > 0 ) return fmin2(1.0, (D/Dmax) );
	else return fmax2(-1.0, (D/Dmax));
}

/*###############################################################
* implementation of r^2
*/
double Rsquare(double tab[3][3])
{
	double D=0, r2=0;
	...
	r2= pow(D,2)/(tab[0][2]*tab[1][2]*tab[2][0]*tab[2][1]);
