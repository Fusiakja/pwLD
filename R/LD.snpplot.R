`LD.snpplot` <- function(valuematrix, annotation, values =c("values", "upperCI", "lowerCI"), LD=c("D", "Dprime", "Q", "r", "OR", "MI", "chi2", "Y", "HS"), snp.zoom=FALSE, snp.target, snp.region, unit=c("Mb","Kb", "bp"))
{
  if(snp.zoom==TRUE)
  {
  ...
  rbPal <- colorRampPalette(c('red','blue'))
  color <- rbPal(10)[as.numeric(cut(abs(y),breaks = 10))]
  plot(x = x,y = y, xlab = paste("Position on chromosome in", unit, sep = " "), ylab = LD, main = paste("LD Region for ", snp.target, sep = ""), pch=17,cex=(abs(y)+0.4), col=color)
