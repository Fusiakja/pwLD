`estCredibleInterval` <-
function( genoCounts, nSim=1000, Dir=rep(1,9), alpha=.05, LD="Q", tol=.Machine$double.eps^.6, digits=12)
{
	ci <- matrix(0, nrow=length(LD), ncol=2)
	rownames(ci) <- LD
	#colnames(ci) <- c( paste(alpha/2*100, "%", sep=""), paste( (1- (alpha/2))*100, "%", sep="" ) )
	
	res <- .C("credibleInterval", genoCounts=as.double(t(genoCounts)),  nSim=as.integer(nSim), Dir=as.double(Dir), LD=as.character(LD), LDnumb=as.integer(length(LD)), tol=as.double(tol), digits=as.integer(digits), LDdist=as.double(numeric(length(LD) * nSim) ), PACKAGE="pwLD")
